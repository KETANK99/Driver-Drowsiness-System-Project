...
class register:
    def __init__(self, root):
        ...
        self.bg = PhotoImage(file="C:\A\AI-For-Road-Safety-main\AI-For-Road-Safety-main\New folder\Img\car1122.jpg")
        ...
        # self.left=ImageTk.PhotoImage(file="car.jpg")
        # left=Label(self.root,image=self.left).place(x=80,y=100,width=400,height=500)

        # Register Here

        frame1 = Frame(self.root, bg="white")
        frame1.place(x=50, y=100, width=700, height=500)

        title = Label(frame1, text="Register Here", font=("time new Roman", 20, "bold"), bg="white", fg="green").place(
            x=50, y=30)

        # ---------------Row 1----------------------
        f_name = Label(frame1, text="First Name", font=("time new Roman", 15, "bold"), bg="white", fg="gray").place(
            x=50, y=100)
        ...
        Email = Label(frame1, text="Enter Mail Id", font=("time new Roman", 15, "bold"), bg="white", fg="gray").place(
            x=370, y=170)
        self.txt_Email = Entry(frame1, font=("time new roman", 15), bg="lightgray")
        self.txt_Email.place(x=370, y=200, width=250)
        # ----------------Row 3---------------------

        address = Label(frame1, text="Enter your Address", font=("time new Roman", 15, "bold"), bg="white",
        ...
        pin = Label(frame1, text="Enter Pin code", font=("time new Roman", 15, "bold"), bg="white", fg="gray").place(
            x=370, y=240)
        self.txt_pin = Entry(frame1, font=("time new roman", 15), bg="lightgray")
        self.txt_pin.place(x=370, y=270, width=250)
        # ----------------Row 4---------------------

        password = Label(frame1, text="Enter your password", font=("time new Roman", 15, "bold"), bg="white",
                         fg="gray").place(x=50, y=310)
        self.txt_password = Entry(frame1, font=("time new roman", 15), bg="lightgray", show='*')
        self.txt_password.place(x=50, y=340, width=250)

        cpass = Label(frame1, text="Confirm Password", font=("time new Roman", 15, "bold"), bg="white",
                      fg="gray").place(x=370, y=310)
        self.txt_cpass = Entry(frame1, font=("time new roman", 15), bg="lightgray", show='*')
        ...
        chk = Checkbutton(frame1, text="I agree the Terms and Conditions", variable=self.var_chk, onvalue=1, offvalue=0,
                          bg="white", font=("time new roman", 12)).place(x=50, y=380)
        ...
        submit_btn = Button(frame1, text="Submit", bg="#d77337", fg="white", font=("time new roman", 20), bd=0,
                            cursor="hand1", command=self.register_data).place(x=50, y=420)
        sign_btn = Button(frame1, text="Sign In", bg="#d77337", fg="white", font=("time new roman", 20), bd=0,
                          cursor="hand1", command=self.login_window).place(x=350, y=420)
    ...
    def register_data(self):
        ...
        if self.txt_fname.get() == "" or self.txt_lname.get() == "" or self.txt_Email.get() == "" or self.txt_address.get() == "" or self.txt_contact.get() == "" or self.txt_pin.get() == "" or self.txt_password.get() == "" or self.txt_cpass.get() == "":
        ...
        #elif self.txt_Email.get() == self.txt_Email.get():
            ...
            try:
                ...
                cur = con.cursor()
                cur.execute(
                    "insert into user1 (f_name,l_name,Email,address,contact,pin,password) values(%s,%s,%s,%s,%s,%s,%s)",
